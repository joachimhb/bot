...
module.exports = {
  ...
  async tag(cwd, name, message) {
    ...
    const tag = await exec('git', {args: ['tag', '-a', `-m "${message}"`, name], cwd});
  ...
  async push(cwd, withTags) {
    check.assert.string(cwd, 'cwd is not a string');

    ui.debug(`Pushing ${cwd}`);

    const push = await exec('git', {args: ['push', 'origin', 'master', withTags ? '--tags' : ''], cwd});
